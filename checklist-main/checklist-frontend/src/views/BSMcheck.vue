...
watchEffect(async()=>{
    ...
    if(authStore.userData?.role == 'BM'){
        router.push(`history/BM/${authStore.userData.location_id}/${authStore.userData.fullname}`)
    }
    const ASM = await sheetAPI.getsheet(fillter.value,'BM').then(res=>{ 
...
<template>
    <div class="flex-col flex px-4 sm:px-6 pt-6 pb-10">
        ...
        <RouterLink v-for="(val,ind) in tmp" :key="ind" :to="{path:`history/BM/${val.ID}/${val.NAME}`}"
