...
import { onMounted, ref, Ref, watchEffect, onUnmounted } from "vue"
...
const information: Ref<information> = ref({ docname: "", status: false, permission: role.value, bu: tmpbu, inform: [{ title: "", formlist: [{ question: "", types: "", timer: [{ time_start: 540, time_end: 720 }], choices: [{ choice: [""], score: 0 }], scorequestion_total: 0 }] }] })
const onSubmit = async () => {
    try {
        ...
        if (isConfirm) {
            ...
            routepush.push(`/manage/${route.params.id}`)
        }
    } catch (error: any) {
        alert(error.message)
    }
    ...
    <div class="flex-col flex px-4 sm:px-6 pt-6 pb-10 overflow-x-hidden">
        <div class="rounded-2xl relative container w-full place-self-center h-full mt-8">
            <div class="h-full container min-h-full">
                <div class="bg-white rounded-2xl px-4 py-5 sm:px-6 sm:shadow-sm">
                        ...
                        <div v-if="menuOpen" ref="menuRef"
                            ...
                            <div class="w-full">
                                <select multiple v-model="tmpbu" @change="tmpbu.push()"
                                    ...
                                    <option value="Istudio"
                                        class="p-2">
                                        <span class="ml-2 text-gray-700"></span>
                                        {{ 'Istudio' }}
                                    </option>
                                    <option value="studio7"
                                        class="p-2">
                                        <span class="ml-2 text-gray-700"></span>
                                        {{ 'studio7' }}
                                    </option>
                    ...
                    <div class="flex flex-row flex-wrap w-full gap-1 mt-3">
                        <div class="flex flex-row rounded-xl bg-sky-300 gap-1 px-2 " v-for="(opt, ops) in tmpbu" :key="ops">
                            <label>{{ opt }}</label>
                            <span @click="tmpbu.splice(ops, 1)" class="rounded-full flex justify-center items-center">
                                ...
                                <div class="grid grid-cols-2 gap-4 mt-4 w-full sm:w-1/2">
                                    <div className="col-span-2 sm:col-span-1">
                                        ...
                                        <select v-model="val.types"
                                            ...
                                            <option value="" >เลือกรูปแบบคำตอบ</option>
            ...
            <div
                ...
                <RouterLink :to="`/manage/${route.params.id}`"
